...
//////////////////////////////////////////////////
///
/// This is really a copy of auxXiRS.c with very tiny modifications to avoid angle averaging.
/// Diff-ing the two files should be useful as an illustration of what one should change when playing with the code.
/// In the end, this code computes the 2D redshift space matter 2-pt function in the Zel'dovich approximation.
/// 
///
//////////////////////////////////////////////////
...
void XIRS2d(double *X,double *res,double *err,double *probability);
void calcXiRS(void);
static Vector3d diagRS;
...
void calcXiRS2d(void)
{
    REDSHIFT_SPACE=1;
    initChiGamma();
    
    double x[2],res,err,prob;
    int i,j;
    ...
    for (i=0;i<=n;i++)
    for (j=0;j<=n;j++){
        x[0]= ((double) i)/((double)n)*(maxX-minX)+minX;
        x[1]= ((double) j)/((double)n)*(maxX-minX)+minX;
        XIRS2d(x,&res,&err,&prob);
        fprintf(file,"%.8e %.8e %.8e %.8e %.8e\n",x[0],x[1],res,err,prob);
    }
    fclose(file);
...
static int Integrand2ptRS(const int *, const double q[], const int *, double f[], void *userdata){
        ...
        phiQ=(q[2]*2.0-1.0)*M_PI; 
        jacPhi=2*M_PI;  // = d phiQ/ d q[3]
        muQ=1.0 - 2.0*q[1]*q[1]*q[1]; 
        jacMuQ=6.0*q[1]*q[1]; // = d \mu/ d q[2]
...
void  XIRS2d(double *X,double *res,double *err,double *probability)
{
       ...
       Xabs=sqrt(x[1]*x[1]+x[0]*x[0]);
       muX=x[1]/Xabs;
       sqX=sqrt(1.0-muX*muX);
       
       void * USERDATA = (void*) x;
       sigmasqrt=sqrt(2.*sigma);
       tanhcoef=tanh(Xabs/(2.*sigmasqrt));
       ...
       fprintf(stderr,"XI_RS l=%i at X=%.5e %.5e:\t%.8e +- %.3e\tp = %.3f\n",MULTIPOLE,X[0],X[1],integral[0],error[0],prob[0]);
